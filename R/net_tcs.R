...
#'
#' @param df_tcs Dataframe. Conversation tweets collected by get_tcs function.
...
#' }
#'
net_tcs <- function(df_tcs = NULL, type = "activity") {
  if (is.null(df_tcs) || !is.data.frame(df_tcs)) {
  ...
  df_tcs %<>% dplyr::rename(tweet_id = .data$id)
  df_tcs %<>% tidyr::unnest(cols = c("referenced_tweets"),
                              keep_empty = TRUE) %>%
    dplyr::rename(
  ...
  if (type == "activity") {
    ...
    nodes <-
      ...
      dplyr::left_join(df_tcs, by = "tweet_id") %>%
    ...
    nodes <-
      ...
      dplyr::left_join(df_tcs, by = c("user_id" = "author_id")) %>%
